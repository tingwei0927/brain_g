...
def count_std(goarray, num):#算標準差
    all = goarray
    std_ = np.std(goarray, axis = 0)
    #print("std",std_)
    #print("std_",std_)
...
def step_top(folderpath, num): #要測的3d nparray資料夾，要找幾個最高點(因為老師說先分別找四肢的最高點就好
    ...
    #print(type(po_RH))
    bigLH, bbbLH, smaLH, sssLH =get_minandmax(po_LH, 1, num )#手的最低點位置跟值，和最高點位置跟值(因為y是越往下越正)
    ...
    LH_B_std =count_std(bbbLH,16)
    LH_S_std =count_std(sssLH,16)
    RH_B_std =count_std(bbbRH,16)
    RH_S_std =count_std(sssRH,16)
    #LF_B_std =count_std(bbbLF,32)
    #LF_S_std =count_std(sssLF,32)
    #RF_B_std =count_std(bbbRF,32)
    #RF_S_std =count_std(sssRF,32)
    ...
    return LH_B_std ,RH_B_std ,LH_S_std ,RH_S_std
    
    
    
    
    
#LH, RH, LF, RF =step_top("./04_2/pic/04_2/np",16)