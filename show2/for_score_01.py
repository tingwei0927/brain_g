...
def stad_count(csv_name): #算次數用
    ...
    # 顯示圖表
    pic_num = csv_name.split("-")[0].split("/")[-1]
    print(pic_num)
    cccc = pic_num
    folder = "E://things/master/DIH/DIH/show2/"+cccc+"/test/"
...
def score01(folder1, txtfile): #用3d keypoint的資料夾讀出所有.npy
    global  sa_H, sa_angle
    output_folder = folder1
    
    vvv = output_folder.split("-")[2]  #因為keypoint路徑通常都是keypoints-3d-01
    ...
    #3D        
    np_path3 = [os.path.join(output_folder,f) for f in os.listdir(output_folder)if f.endswith('.npy')]           
    np_sorted3 = natsorted (np_path3) 
    dist_all3=[]
    for i, file in enumerate(np_sorted3):
        get=np.load(file)
        #print("get",get)
        handsss, angle = fordist(get)
    ...
    csv_name3 = "E://things/master/DIH/DIH/show2/01/test/"+vvv+'-1'
    path3 = csv_name3 + ".csv"
    #print("path3",path3)
...
#             get_H.append(sa_H[i])
    ...
    txt_time = txtfile #讀取存時間的list
    time_tem = [] #用來存時間txt的str的list
    with open(txt_time, "r") as file:
        for line in file.readlines():
            line = line.rstrip('\n') #把換行符號刪掉
            #print(line)
            time_tem.append(line)
    time_list = [] #用來存時間的list
    for item in time_tem:
        time_list.append(float(item))
    elapsed_time = sum(time_list) #影片時間總和
    ...
    LH, RH, LF, RF = for_out.step_top (output_folder,int(howmany/2)) #這次動作的四肢std
...
#score01("E://things/master/DIH/DIH/show2/01/test/keypoints-3d-01","E://things/master/DIH/DIH/show2/01/test/01.txt" )